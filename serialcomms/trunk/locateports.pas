...
(* Return a string describing a hotplug (udev) event, with a maximum length of
  64k bytes plus two terminating zeros. The result will be blank if either the
  netlink socket cannot be prepared, or if no event is read.
*)
function PollHotplugEventUnparsed(): string;

(* Parse the string returned by PollHotplugEventUnparsed() into a stringlist.
*)
function ParseHotplugEvent(unparsed: string): TStringList;

(* Save a string which will be retrieved by PollHotplugEventUnparsed() instead
  of reading from a netlink socket. This will typically contain the minimum
  amount of information needed by a subsequent call to ParseHotplugEvent(),
  rather than the full complement of information generated by the kernel. Return
  false if this cannot be done because there is already a pending pushed event.
*)
function PushHotplugEventUnparsed(const unparsed: string): boolean;

(* Save a message which will be retrieved by PollHotplugEventUnparsed() instead
  of reading from a netlink socket, the ordering of the lines must be correct
  and there may be an optional message= prefix on the first. This will typically
  contain the minimum amount of information needed by a subsequent call to
  ParseHotplugEvent(), rather than the full complement of information generated
  by the kernel. Return false if this cannot be done because there is already a
  pending pushed event, the content of the parameter might be changed but not in
  a way that precludes a retry, the parameter will never be freed.
*)
function PushHotplugEventParsed(const parsed: TStringList): boolean;
...
*)
function PollHotplugEvents(const description: TPortDescription; verbose: boolean= false): boolean;
...
*)
function PollHotplugEvents(verbose: boolean= false): boolean;
...
var
  ...
  pushedHotplugEventUnparsed: string= '';
...
{$ifdef DUMPHOTPLUGEVENTS }
  ...
  begin
    for i := 0 to rcv + 1 do begin
      if i mod 16 = 0 then begin
        if i <> 0 then
          WriteLn(ErrOutput);
        Write(ErrOutput, HexStr(i, 4), '  ')
      end;
      Write(ErrOutput, HexStr(Ord(netLinkBuffer[i]), 2), ' ')
    end;
    WriteLn(ErrOutput);
    WriteLn(ErrOutput);
    Flush(ErrOutput)
  ...
  begin
    for i := 1 to Length(result) do begin
      if i mod 16 = 1 then begin
        if i <> 1 then
          WriteLn(ErrOutput);
        Write(ErrOutput, HexStr((i - 1), 4), '  ')
      end;
      Write(ErrOutput, HexStr(Ord(result[i]), 2), ' ')
    end;
    WriteLn(ErrOutput);
    WriteLn(ErrOutput);
    Flush(ErrOutput)
...
{$endif DUMPHOTPLUGEVENTS }


begin
  if pushedHotplugEventUnparsed <> '' then begin
    result := pushedHotplugEventUnparsed;
    pushedHotplugEventUnparsed := '';
{$ifdef DUMPHOTPLUGEVENTS } dump2 {$endif }
  end else begin
    result := '';
    if udevSocket = invalidSocket then
      udevSocket := prepareUdevSocket();
    if udevSocket <> invalidSocket then begin
...
(* sure we have \0\0 as an unambiguous terminator.                              *)
      ...
      rcv := fpRecv(udevSocket, @netlinkBuffer, SizeOf(netlinkBuffer) - 2, 0);
      if rcv > 0 then begin
{$ifdef DUMPHOTPLUGEVENTS } dump1; {$endif }
        SetLength(result, rcv);         (* Might include trailing zero          *)
        Move(netLinkBuffer, result[1], rcv);
        result := Trim(result);         (* Will contain zero separators         *)
{$ifdef DUMPHOTPLUGEVENTS } dump2; {$endif }
        FillByte(netLinkBuffer, SizeOf(netLinkBuffer), 0)
      end
    end
  end
end { PollHotplugEventUnparsed } ;
...
begin
  if unparsed = '' then
    exit(nil);
  if Pos(#$0#$0, unparsed) > 0 then
    SetLength(unparsed, Pos(#$0#$0, unparsed) - 1);
  unparsed := 'message=' + unparsed;
  result := TStringList.Create;
  result.Delimiter := #$0;
  result.StrictDelimiter := true;       (* Suppress all quoting                 *)
  result.DelimitedText := unparsed;

(* Note that the value associated with the name 'message' is truncated at the   *)
(* first zero, i.e. it's not so much the entire unparsed message as it is the   *)
(* stuff that can't be parsed: in practice it might be the same as the DEVPATH  *)
(* value but that probably shouldn't be relied on. My primary reason for doing  *)
(* it that was was to make sure that the initial value had /some/ name so that  *)
(* all lines in the result were in the same form.                               *)

{$ifdef DUMPHOTPLUGEVENTS } dump {$endif }
end { ParseHotplugEvent } ;
...
begin
  result := pushedHotplugEventUnparsed = '';
  if result then
    pushedHotplugEventUnparsed := unparsed
end { PushHotplugEventUnparsed } ;
...
begin
  if Copy(parsed[0], 1, Length('message=')) = 'message=' then begin
    message := parsed[0];
    Delete(message, 1, Length('message='));
    parsed[0] := message
  end;
  parsed.Delimiter := #$0;
  parsed.StrictDelimiter := true;       (* Suppress all quoting                 *)
  result := PushHotplugEventUnparsed(parsed.DelimitedText)
end { PushHotplugEventParsed } ;
...
(* Receive udev messages indicating hotplug etc. events. In principle these can *)
(* be up to 64K, note two extra bytes left unused in the zeroed buffer to make  *)
(* sure we have \0\0 as an unambiguous terminator should it be needed.          *)

  unparsed := PollHotplugEventUnparsed();
  while unparsed <> '' do begin
    parsed := ParseHotplugEvent(unparsed);
    try
...
(* level, irrespective of whether the description matches. The DEVTYPE test     *)
(* above will allow USB etc. events through, but not HID.                       *)
...
//              if Copy(parsed[k], 1, Length('message=')) <> 'message=' then
                DebugWriteF('%s\n', [parsed[k]])
          end;
...
begin
  result := PollHotplugEvents(Anything, verbose)
end { PollHotplugEvents } ;
